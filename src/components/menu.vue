<template>
    <section id="sideBar" class="nav-list" :class="{'show':showMenu}">
        <section class="list-ul">
            ...
            <router-link class="icon-fenxiang iconfont item" :to="{'name':'Topics',query:{tab:'share'}}">分享</router-link>
            <router-link class="icon-wenda iconfont item" :to="{'name':'Topics',query:{tab:'ask'}}">问答</router-link>
            <router-link class="icon-zhaopin iconfont item" :to="{'name':'Topics',query:{tab:'job'}}">招聘</router-link>
        </section>
    </section>
</template>
<script>
